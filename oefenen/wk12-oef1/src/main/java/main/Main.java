...
public class Main {
    ...
    public static void main(String[] args) throws JaarInToekomstException {
        PutFilmsInDb();
        showMenu();
    }

    private static void showMenu() throws JaarInToekomstException {
    ...
    private static Film stringToFilm(String line) {
        if (line.contains("TV Show"))
            return null;
        String[] split = line.split(";");

        // id, type, title, director, cast, country, date added, release year, rating,
    ...
    private static Set<Regisseur> getRegisseurs(String line, Film film) {
        ...
        for (String s : split) {
            regisseurs++;
            set.add(new Regisseur(s, (Set<Film>) film));
        }
        return set;
    }
}
